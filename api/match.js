...
// Repair truncated JSON by closing unclosed arrays/objects
...
export default async function handler(req, res) {
  ...
  // IP rate limit via Upstash (required — fail closed)
  if (process.env.UPSTASH_REDIS_REST_URL && process.env.UPSTASH_REDIS_REST_TOKEN) {
    ...
    try {
      ...
      // Redis unavailable — fail closed: allow request but log
  ...
  try {
    ...
    // If no closing brace found — response was truncated, attempt repair
    if (end === -1) {
